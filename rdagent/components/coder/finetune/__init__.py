...
def _effective_api_max_workers() -> int:
    """Use the route-aware, bounded data-generation worker count."""
    return effective_api_max_workers()
...
class LLMFinetuneEvolvingStrategy(MultiProcessEvolvingStrategy):
    ...
    def implement_data(
        ...
        system_prompt = T(".prompts:data_coder.system").r(
            ...
            api_max_workers=_effective_api_max_workers(),
    ...
    def implement_lf_config(
        ...
        base_model = target_task.base_model
    ...
    def _generate_llamafactory_config_with_llm(
        ...
        train_config_yaml = normalize_training_config(train_config_yaml, training_policy)
...
class LLMFinetuneCoSTEER(CoSTEER):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Fine-tuning can run against chat-only OpenAI-compatible endpoints.
        # Keep the queried-knowledge structure expected by the evolving
        # strategy, but do not populate it with knowledge that would require
        # an embeddings endpoint on the following loop.
